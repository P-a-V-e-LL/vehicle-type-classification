...
Сохраняет эмбединги выборки данных в файл с расширением .pickle. Для работы программы нужны обученная модель .h5 и выборка данных для сохранения. Пути до них указываются внутри программы.

В началае работы программа прости ввести название файла для сохранения, а затем выбрать вариант записи данных:
- 1 - из каждого класса будет извлечено одно заранее помеченное изображение (пока что сделано в виде сохранения первого изображения);
- 2 - будет предложено выбрать количество изображений из класса в рунчую, при этом программа покажет минимальное и максимально возможное значение длявыбора;
- 3 - из каждого класса будут извлечены и сохранены все доступные изображения.

После работы программы файл с данными будет сохранен в папке embedding_data.

# get_knn.py

Вычисляет принадлежность тестового изображения к классам, записанным в .pickle файл. Для работы программы нужна обученная модель, тестовое изображение и файл с эмбеддингами и их классами. Пути до них указываются внутри программы.

Список выводимых классов по умолчанию 10.
