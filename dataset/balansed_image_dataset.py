"""Кастомный генератор данных для keras с поддержкой safe_triplet"""
...
def balanced_image_dataset_from_directory(
    ...
    **kwargs
    ...
    if labels not in ('inferred', None):
        ...
        if class_names:
            raise ValueError(
                f'You can only pass `class_names` if `labels="inferred"`. Received: labels={labels}, and '
...
def paths_and_labels_to_dataset(
    ...
    paths_datasets = [tf.data.Dataset.from_tensor_slices(x) for x in image_paths_per_label]
    args = (image_size, num_channels, interpolation, augment)
    label_datasets = [dataset_utils.labels_to_dataset(labels, label_mode, num_classes) for labels in labels_per_label]
    ...
    choice_dataset = tf.data.Dataset.from_generator(
        ...
        args=(range(len(unique_labels)), num_classes_per_batch, num_images_per_class, labels_probability, seed)
    )
    ...
    if label_mode is None:
        balanced_img_dataset = balanced_img_dataset.map(lambda x, _: x)
    return balanced_img_dataset


def generator(
    choice_indexes, num_classes_per_batch, num_images_per_class, labels_probability, seed
):
    rng = default_rng(seed=seed)
    while True:
        labels = rng.choice(choice_indexes, num_classes_per_batch, replace=False, p=labels_probability)
...
def aug_fn(image):
    """Функция для корректной работы с albumentations в tf"""
    transform = albumentations.Compose(
        [
            albumentations.HorizontalFlip(p=0.5),
            albumentations.CoarseDropout(
                max_holes=4, min_holes=1, max_height=32, max_width=32, min_height=4, min_width=4, p=0.5
            ),
            # albumentations.OneOf([
            #     albumentations.ImageCompression(),
            #     albumentations.Downscale(scale_min=0.4, scale_max=0.9),
            # ], p=0.2)
        ]
    )
    data = {"image": image}
...
def load_image(
    path, label, image_size, num_channels, interpolation, augment: bool
):
    """Загрузка изображения, аугментация, нормализация [0, 1]"""
    img = tf.io.read_file(path)
    img = tf.image.decode_image(img, channels=num_channels, expand_animations=False)
    img = tf.image.resize(img, size=image_size, method=interpolation)
    if augment:
